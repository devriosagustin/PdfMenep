...
import { FileText, ScanText, Trash2, Upload as UploadIcon } from 'lucide-react';
...
import { downloadNameForOcr, filenameFromContentDisposition } from '@/lib/business/pdf-format';
import {
  MAX_FILENAME_LEN,
  MAX_OCR_BYTES,
  MAX_OCR_PAGES,
...
async function submitForOcr(
  ...
  if (params.pageSelection.trim().length > 0) {
    form.append('pages', params.pageSelection.trim());
  }
  ...
  if (!res.ok) {
    ...
    if (fieldErrors && typeof fieldErrors === 'object') {
      ...
      if (fileMsg) return { ok: false, fieldMessage: fileMsg, toastMessage: fileMsg };
      const firstValue = Object.values(fieldErrors)[0];
      if (firstValue) return { ok: false, fieldMessage: firstValue, toastMessage: firstValue };
    }
  ...
  const fallbackName = downloadNameForOcr(params.file.name);
...
export function OcrPdfClient() {
  const history = useLocalHistory('ocr-pdf');
  const [file, setFile] = useState<File | null>(null);
  const [language, setLanguage] = useState<PdfOcrLanguage>('es');
  ...
  const onAdoptFile = useCallback(async (next: File | null) => {
    ...
    if (next.size > MAX_OCR_BYTES) {
      setFieldMsg(`El archivo supera ${(MAX_OCR_BYTES / (1024 * 1024)).toFixed(0)} MB`);
      setFile(null);
      return;
    }
  ...
  const onSubmit = useCallback(async () => {
    ...
    setPhase('uploading');
    try {
      const result = await submitForOcr(
        { file, language, pageSelection: pagesSelection },
        setPhase,
      );
      if (result.ok) {
        ...
        const note =
          pages !== null
            ...
            : '';
        toast.success(`PDF con OCR${note}: ${result.filename}`, {
  ...
  }, [file, language, pagesSelection, history]);
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-12 sm:py-16">
      <header className="flex flex-col gap-3 text-center">
        ...
        <h1 className="font-display text-h1 font-bold tracking-tight text-foreground">
          Reconocer texto de PDF (OCR)
        </h1>
        <p className="mx-auto max-w-xl text-muted-foreground">
          Sube un PDF escaneado o compuesto solo por imágenes y obtén un PDF con una{' '}
          <span className="font-medium text-foreground">capa de texto buscable</span>. Elige el
          idioma del texto y descarga el archivo con el texto ya reconocible.{' '}
          <span className="font-medium text-foreground">
            Salida: el mismo PDF con texto buscable
          </span>
          .
        </p>
      </header>
      ...
      <Alert className="border-brand-300/60 bg-brand-50/60 text-foreground dark:bg-brand-900/20">
        ...
        <AlertDescription className="text-muted-foreground">
          ...
          máximo:{' '}
          <span className="font-medium text-foreground">
            {(MAX_OCR_BYTES / (1024 * 1024)).toFixed(0)} MB
          </span>
          , hasta {MAX_OCR_PAGES} páginas por operación.
        </AlertDescription>
      </Alert>
      ...
      <Card className="border-border/60 bg-card/80 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Arrastra un PDF o haz clic para seleccionarlo. Después elige el idioma del texto a
            reconocer y, opcionalmente, las páginas a procesar.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <Label
            ...
            className={`flex cursor-pointer flex-col items-center gap-3 rounded-xl border-2 border-dashed px-6 py-10 text-center transition-colors ${
              isDragOver
                ...
                : 'border-brand-300/70 bg-brand-50/30 hover:border-brand-500 hover:bg-brand-50/60 dark:bg-brand-50/0 dark:hover:bg-brand-900/10 dark:hover:bg-brand-900/20'
            }`}
          >
            ...
            <span className="text-xs text-muted-foreground">
              o haz clic para seleccionar — máx. {(MAX_OCR_BYTES / (1024 * 1024)).toFixed(0)} MB,{' '}
              máx. {MAX_OCR_PAGES} páginas
            </span>
            <Input
              id="pdf-ocr-input"
              ref={inputRef}
          ...
          <div className="flex flex-col gap-2">
            <Label htmlFor="pdf-ocr-language" className="text-sm font-medium text-foreground">
              Idioma del texto
            </Label>
            <Select
              value={language}
              onValueChange={(v: string) => setLanguage(v as PdfOcrLanguage)}
              disabled={phase !== 'idle'}
            >
              <SelectTrigger id="pdf-ocr-language" className="w-full sm:w-64">
            ...
            <p className="text-xs text-muted-foreground">
              Elige el idioma del texto del PDF para mejorar la precisión del reconocimiento.
            </p>
          </div>

          <div className="flex flex-col gap-2">
            <Label htmlFor="pdf-ocr-pages" className="text-sm font-medium text-foreground">
              Páginas a reconocer (opcional)
            </Label>
            <Input
              id="pdf-ocr-pages"
              type="text"
              autoComplete="off"
              inputMode="numeric"
              placeholder="Todas las páginas (predeterminado)"
              value={pagesSelection}
              onChange={(e) => setPagesSelection(e.target.value)}
              disabled={phase !== 'idle'}
              className="font-mono text-sm"
            />
            <p className="text-xs text-muted-foreground">
              Déjalo vacío para todas. Si indicas, usa el formato{' '}
              <span className="font-mono">"1,3,5-7"</span> (números y rangos separados por comas).
            </p>
          </div>
          ...
          {phase !== 'idle' && (
            <output className="flex flex-col gap-2">
              ...
              <p className="text-center text-xs font-medium text-brand-700 dark:text-brand-300">
                {phase === 'uploading' ? 'Subiendo PDF…' : 'Reconociendo texto en el servidor…'}
              </p>
            </output>
          )}
          ...
          <p className="text-center text-xs text-muted-foreground">
            El PDF puede pesar hasta {(MAX_OCR_BYTES / (1024 * 1024)).toFixed(0)} MB y contener
            hasta {MAX_OCR_PAGES} páginas.
          </p>
        </CardContent>
      </Card>

      <RecentResultsStrip slug="ocr-pdf" className="w-full" />
    </main>
  );
}
