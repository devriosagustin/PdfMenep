...
import { downloadNameForRepair, filenameFromContentDisposition } from '@/lib/business/pdf-format';
import {
  MAX_FILENAME_LEN,
  MAX_PASSWORD_LEN,
  MAX_REPAIR_BYTES,
...
type Phase = 'idle' | 'uploading' | 'repairing';
...
async function submitForRepair(
  file: File,
  ...
  if (password && password.length > 0) form.append('password', password);

  let res: Response;
  try {
    onPhaseChange('uploading');
    res = await fetch('/api/pdf/repair', {
      method: 'POST',
      body: form,
      cache: 'no-store',
    });
  ...
  const fallbackName = downloadNameForRepair(file.name);
...
export function RepararPdfClient() {
  const history = useLocalHistory('reparar-pdf');
  ...
  const onAdoptFile = useCallback(async (next: File | null) => {
    ...
    if (next.size > MAX_REPAIR_BYTES) {
      setFieldMsg(`El archivo supera ${(MAX_REPAIR_BYTES / (1024 * 1024)).toFixed(0)} MB`);
      setFile(null);
      return;
    }
  ...
  const onSubmit = useCallback(async () => {
    ...
    try {
      ...
      const result = await submitForRepair(file, effectivePassword, setPhase);
      if (result.ok) {
        ...
        toast.success(`PDF reparado${note}: ${result.filename}`, {
  ...
  }, [file, passwordEnabled, password, history.add]);
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-12 sm:py-16">
      <header className="flex flex-col gap-3 text-center">
        ...
        <p className="mx-auto max-w-xl text-muted-foreground">
          Recupera un PDF que no se puede abrir, da error al imprimir o muestra páginas en blanco.
          Lo reescribimos desde cero para que vuelva a abrirse sin perder tu información.{' '}
          <span className="font-medium text-foreground">Salida: el mismo PDF, reparado</span>.
        </p>
      </header>
      ...
      <Alert className="border-brand-300/60 bg-brand-50/60 text-foreground dark:bg-brand-900/20">
        ...
        <AlertDescription className="text-muted-foreground">
          ...
          <span className="font-medium text-foreground">
            {(MAX_REPAIR_BYTES / (1024 * 1024)).toFixed(0)} MB
          </span>
      ...
      <Card className="border-border/60 bg-card/80 shadow-lg">
        <CardHeader>
          <CardTitle>Subir PDF dañado</CardTitle>
          <CardDescription>
            Arrastra un PDF corrupto o haz clic para seleccionarlo. Si está protegido con
            contraseña, márcalo abajo e introdúcela para poder repararlo.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <Label
            htmlFor="reparar-pdf-input"
            onDragOver={(e) => {
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">
              o haz clic para seleccionar — máx. {(MAX_REPAIR_BYTES / (1024 * 1024)).toFixed(0)} MB
            </span>
            <Input
              id="reparar-pdf-input"
              ref={inputRef}
              type="file"
          ...
          <div className="flex items-center justify-between gap-3 rounded-lg border border-border bg-muted/20 px-4 py-3">
            <div className="flex items-start gap-3">
              ...
              <div className="flex flex-col gap-0.5">
                <Label
                  htmlFor="reparar-pdf-password-toggle"
                  className="text-sm font-medium text-foreground"
                >
                  El PDF está protegido con contraseña
                </Label>
                <span className="text-xs text-muted-foreground">
                  Active esta opción solo si el PDF pide una clave al abrirlo.
                </span>
              </div>
            </div>
            <Switch
              id="reparar-pdf-password-toggle"
          ...
          {passwordEnabled && (
            <div className="flex flex-col gap-2">
              <Label htmlFor="reparar-pdf-password" className="text-sm font-medium text-foreground">
                Contraseña del PDF
              </Label>
              ...
              <div className="relative">
                <Input
                  id="reparar-pdf-password"
                  name="reparar-pdf-password"
          ...
          {phase !== 'idle' && (
            <output className="flex flex-col gap-2">
              ...
              <p className="text-center text-xs font-medium text-brand-700 dark:text-brand-300">
                {phase === 'uploading' ? 'Subiendo PDF…' : 'Reparando el PDF en el servidor…'}
              </p>
            </output>
          )}
          ...
          >
            {phase === 'idle' ? 'Reparar PDF' : phase === 'uploading' ? 'Subiendo…' : 'Reparando…'}
          </Button>

          <p className="text-center text-xs text-muted-foreground">
            El PDF puede pesar hasta {(MAX_REPAIR_BYTES / (1024 * 1024)).toFixed(0)} MB. La
            contraseña nunca se almacena ni se guarda en disco.
          </p>
        </CardContent>
      </Card>

      <RecentResultsStrip slug="reparar-pdf" className="w-full" />
    </main>
  );
}
