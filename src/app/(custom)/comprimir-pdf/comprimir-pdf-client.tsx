...
import { downloadNameForOptimize, filenameFromContentDisposition } from '@/lib/business/pdf-format';
import {
  MAX_FILENAME_LEN,
  MAX_OPTIMIZE_BYTES,
  PDF_MAGIC,
...
const LEVELS: Array<{
...
}> = [
  {
    value: 'baja',
    label: 'Baja',
    description: 'menos cambios, mantiene la calidad visualmente idéntica',
  },
  {
    value: 'media',
    label: 'Media',
    description: 'balance entre tamaño y calidad — recomendado',
  },
  {
    value: 'alta',
    label: 'Alta',
    description: 'máxima compresión, puede afectar la nitidez de las imágenes',
  },
];
...
async function submitForOptimize(
  file: File,
  ...
  try {
    ...
    res = await fetch('/api/pdf/optimize', {
      method: 'POST',
      body: form,
      cache: 'no-store',
    });
  ...
  const fallbackName = downloadNameForOptimize(file.name);
...
export function ComprimirPdfClient() {
  const history = useLocalHistory('comprimir-pdf');
  const [file, setFile] = useState<File | null>(null);
  const [level, setLevel] = useState<PdfOptimizeLevel>('media');
  ...
  const onAdoptFile = useCallback(async (next: File | null) => {
    ...
    if (next.size > MAX_OPTIMIZE_BYTES) {
      setFieldMsg(`El archivo supera ${(MAX_OPTIMIZE_BYTES / (1024 * 1024)).toFixed(0)} MB`);
      setFile(null);
      return;
    }
  ...
  const onSubmit = useCallback(async () => {
    ...
    setPhase('uploading');
    try {
      const result = await submitForOptimize(file, level, setPhase);
      if (result.ok) {
        ...
        const note =
          pages !== null
            ...
            : '';
        toast.success(`PDF comprimido${note}: ${result.filename}`, {
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-12 sm:py-16">
      <header className="flex flex-col gap-3 text-center">
        ...
        <p className="mx-auto max-w-xl text-muted-foreground">
          Sube un PDF y elige un nivel de compresión (
          <span className="font-medium text-foreground">Baja, Media o Alta</span>) para reducir su
          peso sin perder calidad visual.{' '}
          <span className="font-medium text-foreground">Salida: un solo PDF más ligero</span>.
        </p>
      </header>
      ...
      <Alert className="border-brand-300/60 bg-brand-50/60 text-foreground dark:bg-brand-900/20">
        ...
        <AlertDescription className="text-muted-foreground">
          ...
          <span className="font-medium text-foreground">
            {(MAX_OPTIMIZE_BYTES / (1024 * 1024)).toFixed(0)} MB
          </span>
      ...
      <Card className="border-border/60 bg-card/80 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Arrastra un PDF o haz clic para seleccionarlo. Elige debajo el nivel de compresión que
            prefieras.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <Label
            htmlFor="pdf-optimize-input"
            onDragOver={(e) => {
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">
              o haz clic para seleccionar — máx. {(MAX_OPTIMIZE_BYTES / (1024 * 1024)).toFixed(0)}{' '}
              MB
            </span>
            <Input
              id="pdf-optimize-input"
              ref={inputRef}
              type="file"
          ...
          <fieldset className="flex flex-col gap-2">
            <legend className="text-sm font-medium text-foreground">Nivel de compresión</legend>
            <RadioGroup
              value={level}
              onValueChange={(v: string) => setLevel(v as PdfOptimizeLevel)}
              disabled={phase !== 'idle'}
              className="grid gap-2"
            >
              {LEVELS.map((opt) => {
                const id = `pdf-optimize-level-${opt.value}`;
                return (
                  <Label
                    htmlFor={id}
                    key={opt.value}
                    className={`flex cursor-pointer items-start gap-3 rounded-lg border border-border bg-muted/20 px-4 py-3 transition-colors hover:border-brand-300 hover:bg-brand-50/40 dark:hover:bg-brand-900/20 ${
                      level === opt.value
          ...
          {phase !== 'idle' && (
            <output className="flex flex-col gap-2">
              ...
              <p className="text-center text-xs font-medium text-brand-700 dark:text-brand-300">
                {phase === 'uploading' ? 'Subiendo PDF…' : 'Comprimiendo PDF en el servidor…'}
              </p>
            </output>
          )}
          ...
          <p className="text-center text-xs text-muted-foreground">
            El PDF puede pesar hasta {(MAX_OPTIMIZE_BYTES / (1024 * 1024)).toFixed(0)} MB. La
            compresión nunca aumenta el tamaño del archivo original.
          </p>
        </CardContent>
      </Card>

      <RecentResultsStrip slug="comprimir-pdf" className="w-full" />
    </main>
  );
}
