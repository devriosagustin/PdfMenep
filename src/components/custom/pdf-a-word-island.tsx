...
import {
  downloadNameForPdfToWord,
  filenameFromContentDisposition,
} from '@/lib/business/pdf-format';
import { MAX_FILENAME_LEN, MAX_UPLOAD_BYTES, PDF_MAGIC } from '@/lib/contracts/pdf-a-word';
...
async function submitForPdfToWord(
  file: File,
  ...
  try {
    ...
    res = await fetch('/api/pdf/word-pdf-convert', {
      method: 'POST',
      body: form,
      cache: 'no-store',
    });
  ...
  const fallbackName = downloadNameForPdfToWord(file.name);
...
export function PdfToWordClient() {
  const history = useLocalHistory('pdf-a-word');
  ...
  const onSubmit = useCallback(async () => {
    ...
    setPhase('uploading');
    try {
      const result = await submitForPdfToWord(file, setPhase);
      if (result.ok) {
        triggerDownload(result.blob, result.filename);
        const pages = result.pages;
        const note = pages !== null ? ` (${pages} ${pages === 1 ? 'página' : 'páginas'})` : '';
        toast.success(`Word generado${note}: ${result.filename}`, {
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-12 sm:py-16">
      <header className="flex flex-col gap-3 text-center">
        ...
        <p className="mx-auto max-w-xl text-muted-foreground">
          Sube un PDF y descarga un <span className="font-medium text-foreground">.docx</span> con
          el texto del PDF organizado por página. Ideal para editar el contenido en tu procesador de
          texto.{' '}
          <span className="font-medium text-foreground">Salida: un archivo Word editable</span>.
        </p>
      </header>
      ...
      <Alert className="border-brand-300/60 bg-brand-50/60 text-foreground dark:bg-brand-900/20">
        ...
        <AlertDescription className="text-muted-foreground">
          ...
          el texto (no el formato completo, imágenes ni tablas).
        </AlertDescription>
      </Alert>
      ...
      <Card className="border-border/60 bg-card/80 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Arrastra un PDF o haz clic para seleccionarlo. Se generará un archivo .docx con el texto
            extraído, manteniendo los saltos de página.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <Label
            htmlFor="pdf-a-word-input"
            onDragOver={(e) => {
          ...
          {phase !== 'idle' && (
            <output className="flex flex-col gap-2">
              ...
              <p className="text-center text-xs font-medium text-brand-700 dark:text-brand-300">
                {phase === 'uploading' ? 'Subiendo PDF…' : 'Generando Word en el servidor…'}
              </p>
            </output>
          )}
      ...
      <RecentResultsStrip slug="pdf-a-word" className="w-full" />
    </main>
  );
}
