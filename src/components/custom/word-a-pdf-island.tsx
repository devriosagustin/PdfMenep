...
import { FileType, Trash2, Upload as UploadIcon } from 'lucide-react';
...
import {
  downloadNameForDocxToPdf,
  filenameFromContentDisposition,
} from '@/lib/business/pdf-format';
import { DOCX_MAGIC, MAX_FILENAME_LEN, MAX_UPLOAD_BYTES } from '@/lib/contracts/word-a-pdf';
...
type Phase = 'idle' | 'uploading' | 'converting';
...
async function readDocxMagic(file: File): Promise<boolean> {
  if (file.size < DOCX_MAGIC.length) return false;
  const slice = file.slice(0, DOCX_MAGIC.length);
  const buf = new Uint8Array(await slice.arrayBuffer());
  for (let i = 0; i < DOCX_MAGIC.length; i++) {
    if (buf[i] !== DOCX_MAGIC[i]) return false;
  }
  return true;
}
...
async function submitForWordToPdf(
  file: File,
  ...
  try {
    ...
    res = await fetch('/api/pdf/word-decompose-convert', {
      method: 'POST',
      body: form,
      cache: 'no-store',
    });
  ...
  const fallbackName = downloadNameForDocxToPdf(file.name);
...
export function WordToPdfClient() {
  const history = useLocalHistory('word-a-pdf');
  ...
  const onAdoptFile = useCallback(async (next: File | null) => {
    ...
    if (next.size > MAX_UPLOAD_BYTES) {
      setFieldMsg(`El archivo supera ${(MAX_UPLOAD_BYTES / (1024 * 1024)).toFixed(0)} MB`);
      setFile(null);
      return;
    }
    const valid = await readDocxMagic(next);
    if (!valid) {
      setFieldMsg('El archivo no es un DOCX válido');
      setFile(null);
      return;
    }
  }, []);
  ...
  const onSubmit = useCallback(async () => {
    ...
    setPhase('uploading');
    try {
      const result = await submitForWordToPdf(file, setPhase);
      if (result.ok) {
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-12 sm:py-16">
      <header className="flex flex-col gap-3 text-center">
        ...
        <h1 className="font-display text-h1 font-bold tracking-tight text-foreground">
          Conversor Word a PDF
        </h1>
        <p className="mx-auto max-w-xl text-muted-foreground">
          Sube un <span className="font-medium text-foreground">.docx</span> con texto y descarga un{' '}
          <span className="font-medium text-foreground">PDF</span> listo para compartir. Ideal para
          documentos simples (sólo texto).{' '}
          <span className="font-medium text-foreground">Salida: un PDF descargable</span>.
        </p>
      </header>
      ...
      <Alert className="border-brand-300/60 bg-brand-50/60 text-foreground dark:bg-brand-900/20">
        ...
        <AlertDescription className="text-muted-foreground">
          Tu DOCX se procesa en el servidor y{' '}
          <span className="font-medium text-foreground">nunca se guarda en disco</span>. Se conserva
          el texto (no imágenes, tablas ni formato complejo). Tamaño máximo:{' '}
      ...
      <Card className="border-border/60 bg-card/80 shadow-lg">
        <CardHeader>
          <CardTitle>Subir DOCX</CardTitle>
          <CardDescription>
            Arrastra un archivo .docx (texto) o haz clic para seleccionarlo. Convertiremos su texto
            a un PDF descargable.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <Label
            htmlFor="word-a-pdf-input"
            onDragOver={(e) => {
          ...
          >
            ...
            <span className="text-sm font-medium text-brand-700 dark:text-brand-300">
              {file ? 'Cambiar archivo' : 'Arrastra tu DOCX aquí'}
            </span>
          ...
          {phase !== 'idle' && (
            <output className="flex flex-col gap-2">
              ...
              <p className="text-center text-xs font-medium text-brand-700 dark:text-brand-300">
                {phase === 'uploading' ? 'Subiendo DOCX…' : 'Generando PDF en el servidor…'}
              </p>
            </output>
          )}
      ...
      <RecentResultsStrip slug="word-a-pdf" className="w-full" />
    </main>
  );
}
