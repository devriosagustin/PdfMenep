...
import { FileText, FileType, Trash2, Upload as UploadIcon } from 'lucide-react';
...
async function submitForExtractText(
  file: File,
  ...
  try {
    ...
    res = await fetch('/api/pdf/extract-text', {
      method: 'POST',
      body: form,
      cache: 'no-store',
    });
  ...
  const fallbackName = downloadNameForExtractText(file.name);
...
export function ExtraerTextoPdfClient() {
  const history = useLocalHistory('extraer-texto-pdf');
  ...
  const onSubmit = useCallback(async () => {
    ...
    setPhase('uploading');
    try {
      const result = await submitForExtractText(file, setPhase);
      if (result.ok) {
        ...
        const note =
          pages !== null
            ...
            : '';
        toast.success(`Texto extraído${note}: ${result.filename}`, {
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-12 sm:py-16">
      <header className="flex flex-col gap-3 text-center">
        ...
        <p className="mx-auto max-w-xl text-muted-foreground">
          Sube un PDF y obtendrás todo su texto en un{' '}
          <span className="font-medium text-foreground">.txt</span>, con cada página separada por un
          salto de página. Ideal para copiar texto, pegar en un editor o pasar a tu sistema de
          búsqueda.{' '}
          <span className="font-medium text-foreground">Salida: un archivo de texto plano</span>.
        </p>
      </header>
      ...
      <Alert className="border-brand-300/60 bg-brand-50/60 text-foreground dark:bg-brand-900/20">
        ...
        <AlertDescription className="text-muted-foreground">
          ...
          , hasta {MAX_PAGES} páginas por operación.
        </AlertDescription>
      </Alert>
      ...
      <Card className="border-border/60 bg-card/80 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Arrastra un PDF o haz clic para seleccionarlo. Te devolveremos su texto en un archivo
            .txt con un salto de página entre cada página del original.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <Label
            htmlFor="pdf-extract-text-input"
            onDragOver={(e) => {
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">
              ...
              máx. {MAX_PAGES} páginas
            </span>
            <Input
              id="pdf-extract-text-input"
              ref={inputRef}
              type="file"
          ...
          {phase !== 'idle' && (
            <output className="flex flex-col gap-2">
              ...
              <p className="text-center text-xs font-medium text-brand-700 dark:text-brand-300">
                {phase === 'uploading' ? 'Subiendo PDF…' : 'Extrayendo texto en el servidor…'}
              </p>
            </output>
          )}
          ...
          <p className="text-center text-xs text-muted-foreground">
            El PDF puede pesar hasta {(MAX_UPLOAD_BYTES / (1024 * 1024)).toFixed(0)} MB y contener
            hasta {MAX_PAGES} páginas.
          </p>
        </CardContent>
      </Card>

      <RecentResultsStrip slug="extraer-texto-pdf" className="w-full" />
    </main>
  );
}
