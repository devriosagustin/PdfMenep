...
import { FileSpreadsheet, FileText, Trash2, Upload as UploadIcon } from 'lucide-react';
...
async function submitForPdfToExcel(
  file: File,
  ...
  try {
    ...
    res = await fetch('/api/pdf/excel-convert', {
      method: 'POST',
      body: form,
      cache: 'no-store',
    });
  ...
  const fallbackName = downloadNameForPdfToExcel(file.name);
...
export function PdfToExcelClient() {
  const history = useLocalHistory('pdf-a-excel');
  ...
  const onSubmit = useCallback(async () => {
    ...
    setPhase('uploading');
    try {
      const result = await submitForPdfToExcel(file, setPhase);
      if (result.ok) {
        ...
        const note =
          pages !== null
            ...
            : '';
        toast.success(`Excel generado${note}: ${result.filename}`, {
    ...
    <main className="mx-auto flex w-full max-w-3xl flex-col gap-8 px-6 py-12 sm:py-16">
      <header className="flex flex-col gap-3 text-center">
        ...
        <p className="mx-auto max-w-xl text-muted-foreground">
          Sube un PDF y descarga un <span className="font-medium text-foreground">.xlsx</span> con
          el texto del PDF en filas y columnas, una fila por cada línea extraída y con la página de
          origen marcada en la primera columna.{' '}
          <span className="font-medium text-foreground">
            Salida: una hoja Excel con el texto estructurado
          </span>
          .
        </p>
      </header>
      ...
      <Alert className="border-brand-300/60 bg-brand-50/60 text-foreground dark:bg-brand-900/20">
        ...
        <AlertDescription className="text-muted-foreground">
          ...
          <span className="font-medium text-foreground">nunca se guarda en disco</span>. Solo se
          recupera el texto (no formato, imágenes ni tablas reales).
        </AlertDescription>
      </Alert>
      ...
      <Card className="border-border/60 bg-card/80 shadow-lg">
        <CardHeader>
          ...
          <CardDescription>
            Arrastra un PDF o haz clic para seleccionarlo. Se generará un archivo .xlsx con una fila
            por cada línea de texto extraída y la página de origen en la primera columna.
          </CardDescription>
        </CardHeader>
        <CardContent className="flex flex-col gap-5">
          <Label
            htmlFor="pdf-a-excel-input"
            onDragOver={(e) => {
          ...
          >
            ...
            <span className="text-xs text-muted-foreground">
              o haz clic para seleccionar — máx. {maxMb} MB, máx. {MAX_PAGES} páginas
            </span>
            <Input
              id="pdf-a-excel-input"
              ref={inputRef}
              type="file"
          ...
          {phase !== 'idle' && (
            <output className="flex flex-col gap-2">
              ...
              <p className="text-center text-xs font-medium text-brand-700 dark:text-brand-300">
                {phase === 'uploading' ? 'Subiendo PDF…' : 'Generando Excel en el servidor…'}
              </p>
            </output>
          )}
          ...
          <p className="text-center text-xs text-muted-foreground">
            Procesamiento privado y sin marca de agua. Tamaño máx. {maxMb} MB, hasta {MAX_PAGES}{' '}
            páginas por operación.
          </p>
        </CardContent>
      </Card>

      <RecentResultsStrip slug="pdf-a-excel" className="w-full" />
    </main>
  );
}
